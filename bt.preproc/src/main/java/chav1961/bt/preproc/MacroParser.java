...
import chav1961.bt.preproc.InternalParser.LexType;
import chav1961.bt.preproc.InternalParser.Lexema;
import chav1961.bt.preproc.InternalParser.Priority;
...
public class MacroParser {
	...
	public boolean next(final Line line) throws IOException {
		if (!rdr.next(line)) {
			...
			try {
				parse(line, classifyLine(line), cl);
	...
	int parse(final Line line, final OperatorType type, final CommandList cl) throws SyntaxException, IOException {
		int		from = 0;
	...
	private static boolean isArrayValid(final char[] toTest, final boolean zeroLength) {
		return toTest != null && (zeroLength || toTest.length > 0);
	}

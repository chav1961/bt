...
loop:		for(int y = 0; y < maxY; y++) {
	...
	public Matrix assign(final Matrix content) {
		if (content == null) {
			...
			return assign(content.extractDoubles());
		}
		else {
			return assign(getTotalPiece(), content);
		}
	...
	public Matrix mulFrom(final Matrix content) {
		if (content == null) {
			...
			final DoubleRealMatrix	result = new DoubleRealMatrix(content.numberOfRows(), this.numberOfColumns());
			final double[]			source = this.content;
			final double[]			target = result.content;
	...
	public Matrix tensorMul(final Matrix content) {
		if (content == null) {
			...
			final DoubleRealMatrix	result = new DoubleRealMatrix(this.numberOfRows() * content.numberOfRows(), this.numberOfColumns() * content.numberOfColumns());
			final double[]			source = this.content;
			final double[]			target = result.content;
				...
				case REAL_DOUBLE	:
					...
					for (int y1 = 0; y1 < maxY1; y1++) {
						for (int x1 = 0; x1 < maxX1; x1++) {
							final double	k = source[y1 * maxX1 + x1];
		...
		if (content == null) {
			...
			switch (content.getType()) {
				...
				case REAL_FLOAT		:
					...
					for (int y1 = 0; y1 < maxY1; y1++) {
						for (int x1 = 0; x1 < maxX1; x1++) {
							final double	k = tempF[y1 * maxX1 + x1];
				...
				case REAL_INT		:
					...
					for (int y1 = 0; y1 < maxY1; y1++) {
						for (int x1 = 0; x1 < maxX1; x1++) {
							final double	k = tempI[y1 * maxX1 + x1];
				...
				case REAL_LONG		:
					...
					for (int y1 = 0; y1 < maxY1; y1++) {
						for (int x1 = 0; x1 < maxX1; x1++) {
							final double	k = tempL[y1 * maxX1 + x1];
	...
	public Matrix invert() {
		if (numberOfRows() != numberOfColumns()) {
			...
			for(int y = 0; y < colSize; y++) {
				final double	k = 1/ source[y * (colSize + 1)];	// Take diagonal element.
	...
	public Matrix apply(final Piece piece, final ApplyFloat callback) {
		if (piece == null) {
			...
			for(int y = 0; y < maxY; y++) {
				for(int x = 0; x < maxX; x++) {
					target[(y0 + y)*numberOfColumns() + (x0 + x)] = callback.apply(y0 + y, x0 + x, (float)(source[(y0 + y)*numberOfColumns() + (x0 + x)]));
				}
			}
			result.completed = false;
			return result;

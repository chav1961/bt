...
public class FloatComplexMatrix implements Matrix {
		...
		if (piece == null) {
			...
			for(int y = 0; y < maxY; y++) {
				for(int x = 0; x < maxX; x++) {
					result[where++] = (int)source[from = ((y0 + y)*numberOfColumns() + (x0 + x)) * 2];
					result[where++] = (int)source[from + 1];
				}
			}
			return result;
		}
	...
	public float[] extractFloats(final Piece piece) {
		if (piece == null) {
			...
			for(int y = 0; y < maxY; y++) {
				for(int x = 0; x < maxX; x++) {
					result[where++] = source[from = ((y0 + y)*numberOfColumns() + (x0 + x)) * 2];
					result[where++] = source[from + 1];
				}
			}
			return result;
		}
	...
	public double[] extractDoubles(final Piece piece) {
		if (piece == null) {
			...
			for(int y = 0; y < maxY; y++) {
				for(int x = 0; x < maxX; x++) {
					result[where++] = (double)source[from = ((y0 + y)*numberOfColumns() + (x0 + x)) * 2];
					result[where++] = (double)source[from + 1];
				}
			}
			return result;
		}
	...
	public Matrix assign(final Piece piece, final int... content) {
		if (piece == null) {
			...
			for(int y = 0; y < maxY; y++) {
				for(int x = 0; x < maxX; x++) {
					result[from = ((y0 + y)*numberOfColumns() + (x0 + x)) * 2] = (float)content[where++];
					result[from + 1] = (float)content[where++];
				}
			}
			return this;
		}
